...
use lisp::{LispObject, MOST_NEGATIVE_FIXNUM, MOST_POSITIVE_FIXNUM};
use remacs_sys::{EmacsDouble, EmacsInt, EmacsUint, Lisp_Object, Qnumberp, Qarith_error,
                 Qrange_error, wrong_type_argument, build_string};
use remacs_sys::libm;
...
pub fn fmod_float(x: LispObject, y: LispObject) -> LispObject {
    let mut f1 = x.any_to_float_or_error();
    let f2 = y.any_to_float_or_error();
